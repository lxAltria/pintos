...
thread_tick (void) 
{
  ...
  {
    struct list_elem *e;
    int64_t currentTick = timer_ticks();
...
static bool
awakeTime_less (const struct list_elem *a_, const struct list_elem *b_,
            void *aux UNUSED) 
{
...
{
  list_insert_ordered(&sleep_list, &t->sleep_elem, awakeTime_less, NULL);
}
